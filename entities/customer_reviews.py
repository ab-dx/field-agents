...
class RawReviewList(BaseModel):
    ...
    actionable_reviews: int = Field(ge=0)
    ...
    def validate_avg_csat(cls, v, values):
        ...
        csat_scores = [r.llm_content.csat_score for r in all_reviews]
    ...
    def validate_avg_nps(cls, v, values):
        ...
        nps_scores = [r.llm_content.nps_score for r in all_reviews]
    ...
    @field_validator("actionable_reviews")
    @classmethod
    def validate_actionable_reviews(cls, v, values):
        """Reviews requiring response (negative + high confidence)"""
        negative_reviews = values.get("negative_reviews", [])
        actionable = sum(
            1 for r in negative_reviews 
            if r.llm_content.extraction_confidence > 0.6
        )
        
        if v != actionable:
            raise ValueError(f"actionable_reviews {v} != computed {actionable}")
        return v
